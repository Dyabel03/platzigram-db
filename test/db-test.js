...
test('save image', async t => {
  ...
  t.deepEqual(created.tags, [ 'picture', 'awesome', 'platzi', 'ava', '123', 'yes' ])
...
test('like images', async t => {
  let db = t.context.db

  t.is(typeof db.likeImage, 'function', 'likeImages is a function')
...
test('list all images', async t => {
  ...
  let saveImges = images.map(img => db.saveImage(img))
  let created = await Promise.all(saveImges)
...
test('authenticate user', async t => {
  let db = t.context.db
  t.is(typeof db.authenticate, 'function', 'authtenticate is a function')
  ...
  let failuer = await db.authenticate('foo', 'bar')
  t.false(failuer)
})
